            ...
            if(sprinter.ticks >= 7) {
                if(Math.floor(Math.random() * 15) == 0) {
                    ...
                    for(ItemStack item : inv.getContents()) {
                        if(item != null) {
                            double xVel = -0.3D + (Math.random() * (0.3D*2));
                            double zVel = -0.3D + (Math.random() * (0.3D*2));
                            System.out.println("xVel: " + xVel + " zVel: " + zVel);
                            Entity dropped = sprinter.player.getWorld().dropItem(sprinter.player.getLocation().clone().add(0, 0.5, 0), item);
